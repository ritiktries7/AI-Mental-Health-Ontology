...
def ontology_explain(phq9: Optional[List[int]], text: str):
    """Create a simple ontology-based explanation using the loaded ontology if available."""
    # Map PHQ-9 indices to meaningful symptom labels (deterministic mapping)
    symptom_map = {
        0: "Loss of interest",
        1: "Sadness",
        2: "Sleep issues",
    ...
    text_cue_map = {
        ...
        "hungry": "Appetite change",
        "worthless": "Low self-worth",
        "alone": "Loneliness",
        "hopeless": "Hopelessness",
        "concentrat": "Concentration issues",
        "interest": "Loss of interest",
        "anhedonia": "Loss of interest",
    }

    # Build PHQ-based explanations using the fixed symptom map (preferred)
    explains = []
    if phq9:
    ...
    # Text cue extraction using text_cue_map. Use a set to dedupe labels.
    cues_set = set()
    txt = (text or "").lower()
    ...
    # If ontology_labels exist and we couldn't find a cue label, we can try a very naive mapping
    # (not used for PHQ mapping) — keep it as additional hints
    cues = sorted(cues_set)

    return {"phq_items": explains, "text_cues": cues}

    # The function returns early above for both PHQ and text cues using the deterministic mapping.
    # Note: if you want additional ontology-derived labels, they are available in
    # app.state.ontology_labels (list) or app.state.ontology (owlready2 ontology) when loaded.
